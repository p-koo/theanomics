...
def dae_model2(input_var, network):

    net = {}
    net['input'] = layers.InputLayer(shape=(None, 970), input_var=input_var)

    # encode layer 1
    net['encode1'] = layers.DenseLayer(net['input'], num_units=3000, W=network['encode1'].W, 
                                      b=network['encode1'].b, nonlinearity=None)
    net['encode1_norm'] = BatchNormLayer(net['encode1'], beta=network['encode1_norm'].beta, gamma=network['encode1_norm'].gamma, mean=network['encode1_norm'].mean, inv_std=network['encode1_norm'].inv_std)
    net['encode1_active'] = layers.NonlinearityLayer(net['encode1_norm'], nonlinearity=nonlinearities.rectify)
    net['encode1_dropout'] = layers.DropoutLayer(net['encode1_active'],p=0.5)

    # encode layer 2
    net['encode2'] = layers.DenseLayer(net['encode1_dropout'], num_units=6000, W=network['encode2'].W, 
                                      b=network['encode2'].b, nonlinearity=None)
    net['encode2_norm'] = BatchNormLayer(net['encode2'], beta=network['encode2_norm'].beta, gamma=network['encode2_norm'].gamma, mean=network['encode2_norm'].mean, inv_std=network['encode2_norm'].inv_std)
    net['encode2_active'] = layers.NonlinearityLayer(net['encode2_norm'], nonlinearity=nonlinearities.rectify)
    net['encode2_dropout'] = layers.DropoutLayer(net['encode2_active'],p=0.5)

    # encode layer
    net['encode'] = layers.DenseLayer(net['encode2_dropout'], num_units=11350, W=network['encode'].W, 
                                      b=network['encode'].b, nonlinearity=None)
...
network2 = dae_model(input_var)

# load model parameters for a given training epoch
best_path = '/home/peter/Data/CMAP/Results/ladder_final_epoch_9.pickle'
f = open(best_path, 'rb')
best_parameters = cPickle.load(f)
f.close()
network2['output'] = layers.set_all_param_values(network2['output'], best_parameters)
all_params = layers.get_all_param_values(network2['encode'])

network = dae_model2(input_var, network2)
#network['output'] = layers.set_all_param_values(network['output'], all_params)
